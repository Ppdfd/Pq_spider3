...
# ─── Graph 9–16: Intra-node Diagnostic Comparisons ────────────────
...
def simulate_intra_node_detailed(
    ...
    views (9, 12, 13, 14, 15).  Running once and plotting multiple views
...
def run_graph8_experiment(
    ...
    All diagnostic graphs (9, 12, 13, 14, 15) plot from these results,
...
def graph12_epc_availability(
    results: Dict[str, Dict[str, np.ndarray]],
    enclaves: List[Enclave],
) -> None:
    """Graph 12: Cumulative EPC Swap Events vs Task Arrivals.
    ...
    ax.set_title("Graph 12: Cumulative EPC Swap Events", fontsize=14)
    ax.set_xlabel("Task Arrival Index", fontsize=12)
    ax.set_ylabel("Total EPC Page Swaps (cumulative)", fontsize=12)
    ax.grid(True, linestyle="--", alpha=0.3)
    ax.legend(fontsize=11)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "graph12_epc_availability.png", dpi=300)
    plt.close(fig)


def graph13_load_imbalance(
    results: Dict[str, Dict[str, np.ndarray]],
    enclaves: List[Enclave],
) -> None:
    """Graph 13: Latency CDF — Cumulative Distribution of Per-Task Latency.
    ...
    ax.set_title("Graph 13: Latency CDF — Per-Task Completion Time", fontsize=14)
    ...
    fig.tight_layout()
    fig.savefig(OUT_DIR / "graph13_load_imbalance.png", dpi=300)
    plt.close(fig)


def graph14_deadline(
    results: Dict[str, Dict[str, np.ndarray]],
    enclaves: List[Enclave],
) -> None:
    """Graph 14: Deadline Compliance — % of Tasks Meeting Deadline.
    ...
    ax.set_title("Graph 14: Deadline Compliance Rate", fontsize=14)
    ...
    ax.grid(True, axis="y", linestyle="--", alpha=0.3)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "graph14_deadline.png", dpi=300)
    plt.close(fig)


def graph15_cache_reuse(
    results: Dict[str, Dict[str, np.ndarray]],
    enclaves: List[Enclave],
) -> None:
    """Graph 15: Cache Affinity — Running Average of Enclave Reuse Count.
    ...
    ax.set_title("Graph 15: Cache Affinity — Enclave Reuse at Selection", fontsize=14)
    ax.set_xlabel("Task Index", fontsize=12)
    ax.set_ylabel("Running Avg. Recent Count (window=20)", fontsize=12)
    ax.grid(True, linestyle="--", alpha=0.3)
    ax.legend(fontsize=11)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "graph15_cache_reuse.png", dpi=300)
    plt.close(fig)


def graph10_contention(rng: np.random.Generator) -> None:
    """Graph 10: Heterogeneity Sensitivity — Latency vs Enclave Speed Spread.
    ...
    plot_lines(
        ...
        "Graph 10: Latency vs Enclave Speed Heterogeneity",
        "Speed Spread (max/min rate ratio)",
        "Average Task Latency (ms)",
        "graph10_contention",
    )
...
def graph11_sensitivity(rng: np.random.Generator) -> None:
    """Graph 11: Parameter Sensitivity Analysis (2-panel).
    ...
    fig.suptitle("Graph 11: Parameter Sensitivity Analysis", fontsize=14, y=1.02)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "graph11_sensitivity.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # Also save raw data
    save_csv(
        RAW_DIR / "graph11_epc_sensitivity.csv",
        "EPC_Swap_Cost_ms", epc_sweep,
        {alg: np.array(panel_a[alg]) for alg in algorithms},
    )
    save_csv(
        RAW_DIR / "graph11_contention_sensitivity.csv",
        "Contention_Per_Unit_ms", cont_sweep,
        {alg: np.array(panel_b[alg]) for alg in algorithms},
    )


def graph16_enclave_scaling(rng: np.random.Generator, reps: int = 10) -> None:
    """Graph 16: Enclave Scaling — 2-panel (Latency + EPC Violations).
    ...
    fig.suptitle("Graph 16: Enclave Scaling Analysis", fontsize=14, fontweight="bold", y=1.01)
    fig.tight_layout()
    fig.savefig(OUT_DIR / "graph16_enclave_scaling.png", dpi=300, bbox_inches="tight")
    plt.close(fig)

    # Save raw CSV data
    save_csv(
        RAW_DIR / "graph16_latency_vs_enclaves.csv",
        "Num_Enclaves", enclave_counts,
        {alg: lat_mean[alg] for alg in algorithms},
    )
    save_csv(
        RAW_DIR / "graph16_epc_violations_vs_enclaves.csv",
        "Num_Enclaves", enclave_counts,
