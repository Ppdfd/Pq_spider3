...
def graph3_intra_enclave(rng: np.random.Generator, reps: int = 10) -> Dict[str, np.ndarray]:
    ...
    N_ENCLAVES = getattr(config, 'G7_NUM_TEES', 4)
    n_tasks = getattr(config, 'G7_NUM_TASKS', 500)

    spread_factors = np.array(getattr(config, 'G7_SPREAD_FACTORS', [1.0, 1.5, 2.0, 2.5, 3.0, 4.0, 5.0]))
...
def run_graph7_experiment(
    ...
    import config
    algorithms = ["Round-Robin", "Least-Queue", "Spider (Ours)"]
    num_tees = getattr(config, 'G7_NUM_TEES', 4)
    num_tasks = getattr(config, 'G7_NUM_TASKS', n_tasks)
