...
Compares three Spider variants under increasing workload intensity:
...
Fixed infrastructure: 20 fog nodes × 4 enclaves per node.
X-axis: offered load (workload intensity), increasing enclave contention.
...
def exp7b_enclave_aware(rng: np.random.Generator) -> Dict[str, np.ndarray]:
    ...
    for name, strategy in VARIANTS.items():
        ...
        for rep in range(reps):
            ...
            for load in offered_loads:
                seed = GLOBAL_SEED + 77000 + 1000 * rep + int(load * 100)
    ...
    # Save raw data
    save_csv(
        RAW_DIR / "exp7b_enclave_aware.csv",
    ...
    plot_lines(
        ...
        "Exp 7 Scenario 2: Enclave-Aware Scheduling",
