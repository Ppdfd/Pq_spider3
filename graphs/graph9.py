"""
Graph 9: Task Completion Ratio vs Failure Rate
===============================================
PQ-SPIDER2 Section VII-C: Fault-Tolerance and Secure Recovery Evaluation

Measures percentage of workloads successfully completed under increasing
fog-node failure rates across the same 6 strategies as Graph 8.
"""

from typing import Dict

import numpy as np

import config
from utils.eval_utils import summarize_runs, save_csv, plot_lines, RAW_DIR

# Reuse baseline simulation functions from graph8
from graphs.graph8 import BASELINES, _run_scenario


def graph9_task_completion(rng: np.random.Generator, reps: int = None):
    """Graph 9: Task Completion Ratio vs Failure Rate."""
    if reps is None:
        reps = config.G9_REPS
    failure_rates = np.array(config.G9_FAILURE_RATES)
    n_fogs = config.G9_NUM_FOGS

    all_runs = {name: [] for name in BASELINES}

    for rep in range(reps):
        per_baseline = {name: [] for name in BASELINES}
        for fr in failure_rates:
            seed = int(rng.integers(0, 2**31)) + rep * 1000
            scenario = _run_scenario(n_fogs, float(fr), seed)
            for name in BASELINES:
                per_baseline[name].append(
                    scenario[name]["task_completion_ratio"] * 100.0
                )
        for name in BASELINES:
            all_runs[name].append(np.array(per_baseline[name]))

    data = {}
    plot_data = {}
    for name in BASELINES:
        mean, std = summarize_runs(all_runs[name])
        data[name] = mean
        plot_data[name] = (mean, std)

    # X-axis as percentage labels
    x_labels = (failure_rates * 100).astype(int)

    save_csv(RAW_DIR / "graph9_task_completion.csv",
             "Failure Rate (%)", x_labels, data)
    plot_lines(
        x_labels, plot_data,
        "Graph 9: Task Completion Ratio vs Failure Rate",
        "Failure Rate (%)",
        "Task Completion Ratio (%)",
        "graph9_task_completion",
    )
    return data
